...
# 정책 내용이 바뀌면 RuleEngineCache에 남아있는 그 정책의 캐시된 판정 결과도 같이 지워야
# 하므로(안 지우면 옛 내용 기준 판정이 계속 재사용됨), 여기서 바로 무효화한다.
@router.post("/upsert")
async def upsert_policy_cache(data: PolicyCacheUpsert, request: Request):
    ...
    if data.plcyNo:
        get_rule_engine_cache(request).invalidate_policy(data.plcyNo)
    return {"message": "ok"}
...
async def remove_policy_cache(policy_id: int, request: Request):
    ...
    if removed_plcyno:
        get_rule_engine_cache(request).invalidate_policy(removed_plcyno)
    return {"message": "ok"}
