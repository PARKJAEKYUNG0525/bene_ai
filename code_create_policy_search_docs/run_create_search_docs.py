# watsonx.ai로 정책 검색문서 전체를 생성하는 실행 스크립트 (search_doc_generator_watsonx.py를 배치로 돌린다)
...
def main():
    """정책 목록 중 START~END 범위를 배치로 나눠 검색문서를 생성한다. 이미 생성된
    정책(done_ids)은 건너뛰고, 배치마다 바로 저장하므로 중간에 중단돼도 이어서 실행할 수 있다."""
